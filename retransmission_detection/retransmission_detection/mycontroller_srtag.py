...
class Controller(object):
    ...
    def writeRedirectRules(self, sw, proto, daddr):
        table_entry = self.p4info_helper.buildTableEntry(
    ...
    def writeLastRedirectRules(self, sw, proto, daddr, eth_addr, out_port):
        table_entry = self.p4info_helper.buildTableEntry(
        table_name="MyIngress.srtag_exact",
        match_fields={
            "hdr.ipv4.protocol": proto,
        ...
        action_name="MyIngress.remove_miss_tag",
        action_params={
            "dstAddr": eth_addr,
            "port": out_port 
        })
    ...
    def writeIDSVerificationRules(self, sw, in_port, val):
        table_entry = self.p4info_helper.buildTableEntry(
        table_name="MyIngress.ids_verification",
        match_fields={
            "standard_metadata.ingress_port": in_port,
        },
        action_name="MyIngress.add_ids_tag",
        action_params={
            "val": val
        })
        sw.WriteTableEntry(table_entry)
    ...
    def writeMarkFlowRules(self, sw, sw_id, dst_addr, out_port):
        table_entry = self.p4info_helper.buildTableEntry(
        table_name="MyIngress.marked_flows",
        match_fields={
            "meta.markForIDS": 1
        },
    ...
    def writeIDSClearRules(self, sw, ip_proto, dst_ip, val):
        table_entry = self.p4info_helper.buildTableEntry(
    ...
    def writeLastIDSClearRules(self, sw, ip_proto, dst_ip, val):
        table_entry = self.p4info_helper.buildTableEntry(
        table_name="MyIngress.ids_clear",
        match_fields={
            "hdr.ipv4.protocol": ip_proto,
        ...
        action_name="MyIngress.remove_ids_tag",
        action_params={
        })
        sw.WriteTableEntry(table_entry)
        print("Install ids clear rule on %s" % sw.name)
    ...
    def start(self):
        ...
        try:
            ...
            self.writeCloneRules(s1, s1.device_id, ids_ip, S1_S3_PORT, h1_ip)
            ...
            self.writeCloneRules(s2, s2.device_id, ids_ip, S2_S3_PORT, h2_ip)
            self.writeCloneRules(s2, s2.device_id, ids_ip, S2_S3_PORT, h3_ip)
            ...
            self.writeRedirectRules(s1, SRTAG_TYPE, ids_ip) 
            ...
            self.writeCloneRules(s3, s3.device_id, ids_ip, SWITCH_TO_HOST_PORT, ids_ip)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--p4info', help='p4info proto in text format from p4c',
                        ...
                        default='./build/basic_srtag.p4.p4info.txt')
