...
class Controller(Iface):
    ...
    def allow(self, req, sw):
        if req != None:
            ...
            if not self.is_existing_flow(req.srcip, req.dstip, proto, sport, dport):
                resp_sw = self.get_resp_switch(req.srcip, req.dstip)
                self.deploy_flow_id_rules(resp_sw, req.srcip, req.dstip, proto)
                self.deploy_ex_port_rules(resp_sw, req.srcip, req.dstip, sport, dport)
                self.add_history_entry(req.srcip, req.dstip, proto, sport, dport, resp_sw) 
        else:
            err = IDSControllerException(1, "cannot read request")
            raise err 

    def is_existing_flow(self, srcip, dstip, srcport, dstport, proto):
        return (((srcip, dstip, proto, srcport, dstport) in self.history) or 
                ((dstip, srcip, proto, dstport, srcport) in self.history))
